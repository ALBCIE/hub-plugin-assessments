...
class AssessmentsEndpoint extends BaseEndpoint {
    ...
    public function __construct()
    {
        ...
        // Set Properties
        $this->required = ['service','importer'];
    }
    ...
    public function createAction(): array
    {
        ...
        if($message['status'] == 200){
            ...
            if($this->Helper->Core->isInstalled('event')){
                ...
                $event = [
                    'category' => 'Assessment',
                    'message' => 'New Assessment Created for <vcard>'.$message['data']['record']['vcard']['name'].'</vcard>',
                    'icon' => 'circle',
                    'color' => 'secondary',
            ...
            if($this->Helper->Core->isInstalled('clients')){
                ...
                // Add the assessment to the client
                $this->Model->Clients->update($message['data']['record']['client']['id'],['assessment' => $message['data']['record']['id']]);
            }
            ...
            if($this->Helper->Core->isInstalled('tasks')){
                ...
                // Retrieve the assessment process
                $process = $this->Model->Process->fetchByTable('assessments');
                ...
                $record['link'] = '/assessment/details?id='.$message['data']['record']['id'].'&name='.urlencode($message['data']['record']['vcard']['name']);
                $record['isActive'] = 0;
                $record['targetTable'] = 'assessments';
                ...
                if($this->Helper->Core->isInstalled('event')){
                    ...
                    $event = [
                        ...
                        'icon' => 'circle',
                        'color' => 'secondary',
                        'link' => '/assessment/details?id='.$message['data']['record']['id'].'&name='.urlencode($message['data']['record']['vcard']['name']),
                        'targetTable' => 'assessments',
                        'targetId' => $message['data']['record']['id'],
                    ];
    ...
    public function updateAction(): array
    {
        ...
        if($message['status'] == 200){
            ...
            if($this->Helper->Core->isInstalled('event')){
                ...
                $event = [
                    'category' => 'Assessment',
                    'message' => 'Assessment Updated for <vcard>'.$message['data']['record']['vcard']['id'].':'.$message['data']['record']['vcard']['name'].'</vcard>',
                    'icon' => 'circle',
                    'color' => 'secondary',
                    'link' => '/crm/details?id='.$message['data']['record']['id'].'&name='.urlencode($message['data']['record']['vcard']['name']),
                    'targetTable' => 'assessments',
    ...
    public function deleteAction(): array
    {
        ...
        if($message['status'] == 200){
            ...
            if($this->Helper->Core->isInstalled('event')){
                ...
                $event = [
                    'category' => 'Assessment',
                    'message' => 'Assessment Deleted for <vcard>'.$message['data']['record']['vcard']['id'].':'.$message['data']['record']['vcard']['name'].'</vcard> by <vcard>'.$this->Auth->user()->vcard['id'].':'.$this->Auth->user()->username.'</vcard>',
                    'icon' => 'circle',
                    'color' => 'secondary',
                    'link' => '/crm/details?id='.$message['data']['record']['id'].'&name='.urlencode($message['data']['record']['vcard']['name']),
                    'targetTable' => 'assessments',
    ...
    public function archiveAction(): array
    {
        ...
        if($message['status'] == 200){
            ...
            if($this->Helper->Core->isInstalled('event')){
                ...
                $event = [
                    'category' => 'Assessment',
                    'message' => 'Assessment Archived for <vcard>'.$record['vcard']['id'].':'.$record['vcard']['name'].'</vcard> by <vcard>'.$this->Auth->user()->vcard['id'].':'.$this->Auth->user()->username.'</vcard>',
                    'icon' => 'circle',
                    'color' => 'secondary',
    ...
    public function recoverAction(): array
    {
        ...
        if($message['status'] == 200){
            ...
            if($this->Helper->Core->isInstalled('event')){
                ...
                $event = [
                    'category' => 'Assessment',
                    'message' => 'Assessment Recovered for <vcard>'.$message['data']['record']['vcard']['id'].':'.$message['data']['record']['vcard']['name'].'</vcard> by <vcard>'.$this->Auth->user()->vcard['id'].':'.$this->Auth->user()->username.'</vcard>',
                    'icon' => 'circle',
                    'color' => 'secondary',
                    'link' => '/crm/details?id='.$message['data']['record']['id'].'&name='.urlencode($message['data']['record']['vcard']['name']),
                    'targetTable' => 'assessments',
